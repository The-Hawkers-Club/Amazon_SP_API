# Cross-repo instruction — XR-IN-1000810 from estatehub

> Filed via `xrepo-relay.py request`, tracked as `XR-IN-1000810` on this repo's own ledger — not a founder brain dump.
> This is ALREADY AUTHORIZED, buildable work: the sending repo made this ask deliberately, having already decided it belongs here. Plan it as instructed work; only park a question if the instruction is genuinely ambiguous about something the FOUNDER, not the sender, must decide — a technical detail the sender could have specified is not that.
